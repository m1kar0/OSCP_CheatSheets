...
- [ ] Find all subdomains and domains related to the audit. 
...
Many WAFs block `alert()`.. so instead of it try using `print()` to detect xss injection point or any other non trivial function.
...
This type of XSS is somewhat more complex as it needs some source to get then injected into a sink where it is processed and activated.

The order of finding DOM XSS is as follows:
...
You open the web page and go to the Debugger within dev tools in the browser. Then do CTRL+F to look for `<script`. Then you may find some interesting script which is you SINK.

SINK EXAMPLE (hashchange function is our target):
...
So obviously we need to trigger this `hashchange` function. How? By looking for SOURCE. Which is `decodeURIComponent`, so

SOURCE is URI:

```text
https://0a35000c03afc49780dc032c00bf0069.web-security-academy.net/#[some html tag]
```

Only thing we need now is to construct some injection scheme:

```html
<iframe src="https://0a35000c03afc49780dc032c00bf0069.web-security-academy.net/#" 
        onload="this.src+='<img src=x onerror=print()>'" 
        hidden="hidden"></iframe>
```

So if victim opens it he gets redirected to the URI with # but without payload. And once redirected the URI gets payload added to it. User is PWNED!
