**SCCM Network Access Account (NAA)** — the NAA is a domain account SCCM configures so that machines which are not joined to the domain can still download software packages. To make that work, SCCM pushes the NAA's cleartext username and password down to every machine **running the SCCM client**, protected only by DPAPI (Windows' built-in per-account secret store) under the `SYSTEM` account's user key — which any local admin or SYSTEM-level attacker can decrypt. Recover it and you get a ready-to-use, often over-privileged domain account. There are two mechanically different paths. **On the host itself** — the local method, and `SystemDPAPIdump.py`, which works like secretsdump — you read the two encrypted `CCM_NetworkAccessAccount` WMI blobs and decrypt them with the SYSTEM DPAPI key. **Remotely** — `sccmhunter.py http` or an NTLM relay — you never touch the host's WMI/RPC: you register or spoof a client and **request the NAA policy from the SCCM management point over HTTP**, then decrypt it.
