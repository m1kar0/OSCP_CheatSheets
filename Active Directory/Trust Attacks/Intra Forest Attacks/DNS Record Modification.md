**DNS record modification / injection** — creating or modifying DNS records is often possible as a domain user (through Active Directory Integrated DNS), and sometimes even unauthenticated (dynamic DNS updates). Useful to register your host name for coercion/relay (see [Coercing Authentication](../../Coercion/Coercing%20Authentication.md)) or to hijack a stale record.
